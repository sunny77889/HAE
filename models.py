...
class AE_Tree():
    ...
    def fit(self, X, k=2/3, epochs=20, batch_size=128, validation_data=None):
        '''
        HAE模型训练
        :param X: 训练数据集
        :param k: 每次数据划分的比例，如认为2/3的数据没有训练好（异常分数高）
        :param epochs:训练迭代次数
        :param batch_size:每次更新参数对应的样本量
        ...
        for clf in self.unfit_clfs:
            ...
            # 选异常分数最高的部分数据作为下一个模型迭代的训练数据，
            bad_samp_len = int(len(loss)*k) #学得不好的样本数目
            idx = np.argpartition(loss, -bad_samp_len)[-bad_samp_len:] #学得不好的样本索引
            X = X[idx]#学得不好的样本
            clf.c = np.min(loss[idx])# 并吧中间的异常分数作为该ae的阈值
...
class ADAE():
    ...
    def fit(self, X, y, epochs=20, batch_size=128, validation_data=None):
        ...
        for clf in self.unfit_clfs:
            ...
            y_pre = clf.predict(X)
            w = ((y-y_pre)**2).mean(axis=1) #样本权重更新
            clf.c = 1 / (0.5 * math.log(np.square(np.sum(w)))) #集成的每个AE对应的权重
            self.clfs.append(clf)
